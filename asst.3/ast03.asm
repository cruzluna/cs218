...
; unsigned or signed, addition is the same
...
; subtraction instructions operate the same on signed and unsigned
; -----
...
;	wAns13  = bNum3 * bNum3
mov al, byte [bNum1]
mul byte [bNum2] ; result in ax b/c al*<op8> = ax
mov word [wAns11], ax

mov al, byte [bNum3] 
mul byte [bNum4]	; result in ax
mov word [wAns12], ax

mov al, byte [bNum3] 
mul al				; result in ax, same as bNum3^2
mov word [wAns13], ax
...
; byte to word
; mov ah, byte [wNum7] ; first half of bits are getting filled
; mov al, byte [wNum7+1]
; idiv byte [bNum6]
; mov byte [bAns21], al
; mov byte [bRem21], ah
...
mov ax, word [wNum1]
mul word [wNum3] 	; result in dx:ax
mov word [dAns11], ax
mov word [dAns11+2], dx

mov ax, word [wNum4]
mul word [wNum1] 	; result in dx:ax
mov word [dAns12], ax
mov word [dAns12+2], dx

mov ax, word [wNum2]
mul word [wNum2] 	; result in dx:ax
mov word [dAns13], ax
...
mov ax, word [wNum6]
imul word [wNum7] 	; result in dx:ax
mov word [dAns14], ax
mov word [dAns14+2], dx

mov ax, word [wNum5]
imul word [wNum8] 	; result in dx:ax
mov word [dAns15], ax
...
;	wRem21 = dNum8 % wNum6
mov ax, word [wNum5]
cwd					; convert word in ax into dobule word in dx:ax
idiv word [wNum7]
mov word [wAns19], ax
...
mov eax, dword [dNum4]
mul dword [dNum1]		; result in edx:eax
mov dword [qAns11], eax
mov dword [qAns11+4], edx

mov eax, dword [dNum3]
mul dword [dNum3]		; result in edx:eax
...
mov eax, dword [dNum7]
imul dword [dNum5]		; result in edx:eax
mov dword [qAns14], eax
mov dword [qAns14+4], edx

mov eax, dword [dNum8]
imul dword [dNum6]		; result in edx:eax
...
mov eax, dword [dNum1]
mov edx, 0 				; 0 b.c unsigned
div dword [dNum3]
mov dword [dAns16], eax

mov eax, dword [dNum4]
mov edx, 0 ; set to zero b/c unsigned
div dword [dNum2]
mov dword [dAns17],eax 

; quadWord to double word
mov eax, dword [qAns12]
mov edx, dword [qAns12+4]	; eax: +4
div dword [dNum3]
...
; convert word into a quad
mov eax, dword [dNum5]
cdq					; signed extension eax to edx:eax
idiv dword [dNum7]
...
mov rax, qword [qNum1]
mul qword [qNum3]	; rdx:rax
mov qword [dqAns11], rax
mov qword [dqAns11+8], rdx 

mov rax, qword [qNum2]
mul qword [qNum4]	; rdx:rax
...
;	dqAns15  = qNum6 * qNum8
mov rax, qword [qNum5]
imul qword [qNum7]	; rdx:rax
mov qword [dqAns14], rax
mov qword [dqAns14+8], rdx 

mov rax, qword [qNum6]
imul qword [qNum8]	; rdx:rax
...
; double quad to quad
mov rax, qword [dqAns12] ; first 8 bytes 
